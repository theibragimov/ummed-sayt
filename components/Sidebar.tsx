...
const nav = [
  ...
  { href: '/earnings', icon: TrendingUp, label: 'Daromad' },
  { href: '/expenses', icon: CreditCard, label: 'Xarajatlar' },
  { href: '/products', icon: Package, label: 'Mahsulotlar' },
  { href: '/clients', icon: Users, label: 'Mijozlar' },
  { href: '/analytics', icon: BarChart2, label: 'Tahlil' },
    ...
    <>
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 h-full w-16 flex-col items-center py-5 z-50 gap-1"
        ...
        <div className="flex flex-col items-center gap-1.5 mt-auto">
          <button title="Yordam" className="w-10 h-10 rounded-xl flex items-center justify-center text-gray-600 hover:text-gray-300 hover:bg-white/8 transition-all"
            style={{}} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.08)')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
            <HelpCircle size={17} />
          </button>
          <button title="Chiqish" className="w-10 h-10 rounded-xl flex items-center justify-center text-gray-600 transition-all"
      ...
      {/* Mobile overlay */}
      {open && (
        <div className="lg:hidden fixed inset-0 z-50 bg-black/60 backdrop-blur-[2px]"
      ...
      <aside className={`lg:hidden fixed left-0 top-0 h-full w-64 z-50 flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] ${open ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="px-3 pb-5 space-y-0.5" style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '12px', marginTop: '8px' }}>
          <button className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-[13px] font-semibold text-gray-600 hover:text-gray-300 transition-all"
            onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.05)')}
            onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
            <HelpCircle size={16} /> Yordam
          </button>
          <button className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-[13px] font-semibold text-red-500 transition-all"
