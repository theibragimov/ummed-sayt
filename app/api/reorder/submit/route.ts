...
function validateOrder(customer: any, items: any[]): string | null {
  ...
  if (!customer?.company || typeof customer.company !== 'string' || customer.company.trim().length < 2) {
    return 'Dorixona nomi noto\'g\'ri';
  }
  if (!customer?.address || typeof customer.address !== 'string' || customer.address.trim().length < 2) {
    return 'Manzil noto\'g\'ri';
  }
...
export async function POST(req: NextRequest) {
  ...
  if (!checkRateLimit(`reorder:${ip}`, 3, 10 * 60 * 1000)) {
  ...
  try {
    ...
    // Get "123" counterparty (dedicated agent for the /reorder link)
    const agentName = '123';
    ...
    console.log(`[REORDER_FAILED]\nRequestID: ${requestId}\nTimestamp: ${timestamp()}\nDuration: ${duration}ms\nReason: ${reason}`);
    console.error('reorder/submit xato:', e);
