...
export async function getMahsulotlar({ kategoriyaSlug, featured, limit, turi } = {}) {
  // "Tez orada" deb belgilangan mahsulotlar hali mavjud bo'lmasa ham ko'rinadi
  const where = { OR: [{ mavjudligi: true }, { belgi: 'tez_orada' }], kategoriyaId: { not: null } }
  if (kategoriyaSlug) where.kategoriya = { slug: kategoriyaSlug }
  if (featured) where.featured = true
  if (turi) where.turi = turi
  else where.turi = 'katalog'
  // Katalog bo'limi endi faqat qo'lda (admin panel orqali) qo'shilgan mahsulotlarni ko'rsatadi —
  // MoySklad'dan avtomatik import qilingan mahsulotlar (moyskladId bor) saytda ko'rinmaydi
  if (where.turi === 'katalog') where.moyskladId = null
  ...
  return prisma.mahsulot.findMany({
    ...
    take: limit,
    include: {
      kategoriya: { select: { nom: true, nomRu: true, nomEn: true, slug: true, rangKodi: true } },
...
export async function getAllMahsulotlar({ turi } = {}) {
  return prisma.mahsulot.findMany({
    ...
    orderBy: { nom: 'asc' },
    select: {
      id: true, nom: true, nomRu: true, nomEn: true, slug: true,
      kategoriyaId: true, asosiyRasmUrl: true, narx: true, mavjudligi: true,
      qisqaTavsif: true, qisqaTavsifRu: true, qisqaTavsifEn: true,
      brend: true, featured: true, belgi: true, turi: true, variantlar: true,
      kategoriya: { select: { id: true, nom: true, nomRu: true, nomEn: true, slug: true, rangKodi: true, tartibRaqami: true } },
      rasmlar: { orderBy: { tartib: 'asc' }, take: 1, select: { rasmUrl: true } },
    },
  })
}
...
export async function getMahsulotBySlug(slug) {
  return prisma.mahsulot.findUnique({
    where: { slug },
    select: {
      id: true, nom: true, nomRu: true, nomEn: true, asosiyRasmUrl: true, videoUrl: true,
      mavjudligi: true, qisqaTavsif: true, qisqaTavsifRu: true, qisqaTavsifEn: true,
      toliqTavsif: true, toliqTavsifRu: true, toliqTavsifEn: true, brend: true, modelRaqami: true,
      featured: true, belgi: true, variantlar: true,
      kategoriya: { select: { nom: true, nomRu: true, nomEn: true, slug: true, rangKodi: true } },
      rasmlar: { orderBy: { tartib: 'asc' }, select: { rasmUrl: true } },
    },
  })
}
...
export async function createMahsulot(data) {
  ...
  let baseSlug = rest.slug || slugYarat(rest.nom)
  let slug = baseSlug
  let n = 1
  while (await prisma.mahsulot.findUnique({ where: { slug } })) {
    slug = `${baseSlug}-${n++}`
  }
...
export async function getPostlar({ limit, kategoriyaSlug, hammasi } = {}) {
  ...
  return prisma.post.findMany({
    ...
    take: limit,
    select: {
      id: true, sarlavha: true, sarlavhaRu: true, slug: true, sana: true,
      muqovaRasmUrl: true, qisqaTavsif: true, qisqaTavsifRu: true,
      kategoriya: { select: { nom: true, rang: true } },
    },
  })
}
...
export async function getAllPostlar() {
  return prisma.post.findMany({
    orderBy: { sana: 'desc' },
    select: {
      id: true, sarlavha: true, muallif: true, sana: true, holat: true,
      kategoriya: { select: { nom: true } },
    },
  })
}
...
export async function getPostBySlug(slug) {
  return prisma.post.findUnique({
    where: { slug },
    select: {
      sarlavha: true, sarlavhaRu: true, sana: true, muallif: true,
      muqovaRasmUrl: true, qisqaTavsif: true, qisqaTavsifRu: true,
      toliqMatn: true, toliqMatnRu: true,
      kategoriya: { select: { nom: true, rang: true } },
    },
  })
}
...
export async function createZayavka(data) {
  const zayavka = await prisma.zayavka.create({ data })
  try {
    await zayavkaniTelegramgaYuborish(zayavka)
  } catch (err) {
...
async function zayavkaniTelegramgaYuborish(z) {
  ...
  const lines = [
    ...
    ``,
    `👤 Ism: <b>${z.ism || '—'}</b>`,
    `📞 Telefon: <b>${z.telefon || '—'}</b>`,
  ]
  if (z.email) lines.push(`✉️ Email: ${z.email}`)
  if (z.mahsulot) lines.push(`📦 Mahsulot: ${z.mahsulot}`)
  if (z.xabar) lines.push(``, `💬 ${z.xabar}`)
