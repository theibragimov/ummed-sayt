...
export async function getMahsulotlar({ kategoriyaSlug, featured, limit } = {}) {
  const where = { mavjudligi: true }
  ...
  return prisma.mahsulot.findMany({
    ...
    take: limit,
    include: {
      kategoriya: { select: { nom: true, slug: true } },
      rasmlar: { orderBy: { tartib: 'asc' }, take: 1 },
    },
  })
}

export async function getAllMahsulotlar() {
  return prisma.mahsulot.findMany({
    orderBy: { yaratilgan: 'desc' },
    include: {
      kategoriya: { select: { nom: true, slug: true } },
...
export async function createPost(data) {
  const slug = data.slug || slugYarat(data.sarlavha)
