...
export async function getMahsulotBySlug(slug) {
  return prisma.mahsulot.findUnique({
    where: { slug },
    select: {
      id: true, nom: true, nomRu: true, nomEn: true, asosiyRasmUrl: true,
      mavjudligi: true, qisqaTavsif: true, qisqaTavsifRu: true,
      toliqTavsif: true, toliqTavsifRu: true, brend: true, modelRaqami: true,
...
export async function createZayavka(data) {
  ...
  zayavkaniTelegramgaYuborish(zayavka).catch(err => console.error('Telegram xabari yuborilmadi:', err))
  return zayavka
}
